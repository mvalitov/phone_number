defmodule PhoneNumber.Country do
  ...
  defstruct(
    ...
    full_general_pattern: nil,
    mobile_token: nil,
    data: %{},
    general_validation: nil,
    validations: [],
      ...
      {:ok, body} ->
        ...
        |> Enum.map(fn territory ->
          national_prefix_for_parsing =
            case xpath(territory, ~x"@nationalPrefixForParsing"s)
                 |> String.replace("\n", "")
                 |> String.replace(" ", "") do
              "" -> nil
          ...
          country = %Country{
            ...
            general_validation:
              Regex.compile!(
                xpath(territory, ~x"./generalDesc/nationalNumberPattern/text()"s)
                |> String.replace("\n", "")
                |> String.replace(" ", "")
              ),
          ...
          %{
            country
            | mobile_token: @mobile_token_countries[country.id],
              full_general_pattern:
                Regex.compile!(
                  "^(#{country.country_code})?(#{country.national_prefix})?(?<national_num>#{
                    country.general_validation.source
                  })$"
                )
          }
        end)
